...
function filterScopedApiFiles(entries: ReportusFileEntry[]): LogFileDescriptor[] {
  const wanted = ['vm.log', 'parallels-system.log', 'tools.log'];
  ...
  for (const entry of entries) {
    ...
    if (entry.path.length < current.path.length) byFilename.set(entry.filename, entry);
  }
