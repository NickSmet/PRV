...
### ReportModel
A unified typed object representing the entire report state. Located in `src/lib/types/report.ts`.
...
### Markers
Typed objects that represent visual indicators. Located in `src/lib/types/markers.ts`.
...
Add the rule to the `currentVmRules` array in `src/lib/rules/currentVm.ts`:
...
│   └── markers.ts      # Marker types, target types, helper functions
├── stores/
│   └── markerStore.svelte.ts  # Reactive marker store, scroll-to-marker
├── rules/
│   ├── types.ts        # Rule type definitions
│   ├── currentVm.ts    # CurrentVm rules (27 rules)
│   └── index.ts        # Rule registry, evaluateRules()
├── components/
│   └── compact/
│       ├── RowValue.svelte       # Reusable row value renderer
│       └── CompactCurrentVm.svelte  # CurrentVm compact layout
└── nodeBuilder.ts      # buildCurrentVmNodeWithMarkers()
```
...
```svelte
<script lang="ts">
  import RowValue from '$lib/components/compact/RowValue.svelte';
</script>
