...
`packages/report-core/src/services/` contains **pure parsers** that convert raw report node payloads (XML / JSON / plain text) into typed summaries consumed by:
- `packages/report-core/src/types/report.ts` (canonical `ReportModel`), and
- `packages/report-viewmodel` (UI-ready `NodeModel[]`)
...
See: `packages/report-core/src/types/SPEC.md`.

## Architecture

```
Full Report XML (ParallelsProblemReport)
  └─ apps/userscript/src/lib/reportLoader.ts
      └─ extracts node payload strings into `window.__prv_*` globals (userscript surface only)
  └─ packages/report-api (Reportus HTTP client)
      └─ downloads attachment/log file contents (web + MCP surfaces)

Node payload (XML/JSON/text)
  └─ packages/report-core/src/services/nodes/*/parse*.ts (per-node implementations)
      └─ returns *Summary | null
  └─ packages/report-core/src/services/index.ts (stable export surface; re-export from nodes/)

Summary / ReportModel
  └─ packages/report-core/src/nodeRegistry.ts
      └─ resolves payload sources by filename hints (Reportus index) and builds `ReportModel`
  └─ packages/report-core/src/rules
      └─ evaluates markers from `ReportModel`

Viewmodels
  └─ packages/report-viewmodel
      └─ builds `NodeModel[]` for rendering
```
...
- Parsers should **not** read from `window.__prv_*` directly — they accept their input as an argument. (Loading/extraction is owned by surface code like `apps/userscript/src/lib/reportLoader.ts` or by `packages/report-core/src/nodeRegistry.ts` + `packages/report-api`.)

### XML access helpers (current + target)

Today, XML parsing is implemented as a mix of:
- direct `DOMParser` usage in some parsers, and
- shared helpers in `packages/report-core/src/services/xmlUtils.ts` in a few parsers.

**Target contract:** XML parsing should be routed through a shared abstraction so the underlying engine can change.

Proposed helper surface (owned by `packages/report-core/src/services/xmlUtils.ts`):
...
Per-node specs live next to implementations under `packages/report-core/src/services/nodes/*/SPEC.md`.
...
1. Update the node spec in `packages/report-core/src/services/nodes/<node>/SPEC.md`
2. Update the parser implementation in `packages/report-core/src/services/nodes/<node>/parse*.ts`
3. Validate with `npm run parse:node` (and optionally snapshot with `--out`)
4. Wire/normalize into `ReportModel` (`packages/report-core/src/types/report.ts`) as needed
...
**Runtime constraints**
- Must run in both browser and Node runtimes. In Node, call `ensureDomParser()` from `@prv/report-core` before running any DOMParser-based parsers.
...
Approach:
1. Expand `packages/report-core/src/services/xmlUtils.ts` into the single entrypoint for XML parsing and selection (tag-path based).
...
4. Keep `apps/userscript/src/lib/reportLoader.ts` parsing strategy independent (it can remain `DOMParser` initially, since it benefits from `querySelector` over the full report document).
...
- Loader/extraction (userscript): `apps/userscript/src/lib/reportLoader.ts`
- Payload resolution (web/MCP): `packages/report-api` + `packages/report-core/src/nodeRegistry.ts`
- Viewmodels: `packages/report-viewmodel`
...
- `packages/report-core/src/types/SPEC.md` — report model + marker types
- `packages/report-core/src/rules/README.md` — rule engine (badges/markers)
- `packages/report-viewmodel/SPEC.md` — NodeModel builder contract
